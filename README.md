...
Deploying agents inside a regulated enterprise is not a model problem, it is a blast-radius problem. StageGate is the control plane for that: a capability starts in shadow mode where its full intended effect is recorded and nothing executes, graduates to human approval, and only then acts — with an append-only audit trail across all three, a kill switch that degrades rather than crashes, and a dry-run report you can hand to whoever has to sign the deployment off.

## Quickstart

```bash
uv pip install -e ".[dev]"     # or: pip install -e ".[dev]"
python -m pytest               # 270 tests, no network, no credentials
